...
function MenuSection() {
  ...
  return (
    <header className={cn("fixed z-50 left-0 right-0 bg-background border-b border-border/25")}>
      <nav className='container py-4 flex justify-between items-center'>
       ...
       <Image src={'/logo.png'} alt={'logo'} width={60} height={45}/>
        ...
        <ul className="md:flex hidden items-center text-[17px] gap-6 font-medium">
            {ROUTES.map((route)=>(
                <li key={route.id}>
                    <Link href={route.path} className={cn("hover:text-primary",path === route.path && 'text-primary')}>{route.name}</Link>
                </li>
            ))}
        </ul>
